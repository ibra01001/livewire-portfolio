    ...
    <div class="bg-white border-2 border-[#1A1A1A] rounded-lg shadow-[4px_4px_0px_0px_#1A1A1A] p-8 max-w-2xl mx-auto">
        <form wire:submit="save" class="space-y-6">
            ...
            <div class="flex items-center space-x-6">
                @if ($experience->company_logo && !$company_logo)
                    <div class="flex-shrink-0">
                        <img src="{{ Storage::url($experience->company_logo) }}"
                            class="h-20 w-20 object-contain rounded-md border-2 border-[#1A1A1A]">
                    </div>
                @endif
                <div class="flex-grow">
                    <label class="block text-sm font-bold text-[#1A1A1A] mb-2 uppercase tracking-wide">Update Logo
                        (Optional)</label>
                    <div
