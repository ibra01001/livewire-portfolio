<div>
    <div class="flex justify-between items-center mb-8">
        <div class="flex items-center space-x-4">
            <a href="{{ route('admin.career.index') }}" class="text-[#1A1A1A] hover:text-[#4ECDC4] transition-colors">
        ...
        <a href="{{ route('admin.career.education.create') }}"
            class="px-5 py-3 bg-[#4ECDC4] text-white font-bold rounded-lg border-2 border-[#1A1A1A] shadow-[4px_4px_0px_0px_#1A1A1A] hover:shadow-none hover:translate-x-[2px] hover:translate-y-[2px] transition-all duration-200 flex items-center"
                ...
                @forelse ($educations as $education)
                    <tr class="hover:bg-[#F5F3EF] transition-colors">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap border-r-2 border-[#1A1A1A]">
                            <div>
                                <div class="text-sm font-bold text-[#1A1A1A]">{{ $education->degree }}</div>
                                <div class="text-xs font-bold text-[#3D3D3D] opacity-60">{{ $education->institution }}</div>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap border-r-2 border-[#1A1A1A]">
                            <button wire:click="toggleActive({{ $education->id }})"
                                class="px-2 py-1 inline-flex text-xs leading-5 font-bold rounded border-2 border-[#1A1A1A] {{ $education->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                {{ $education->is_active ? 'Active' : 'Hidden' }}
                            </button>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <a href="{{ route('admin.career.education.edit', $education) }}"
